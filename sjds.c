...
int main(){
char mapl[] = "qwertyuiopasdfghjklzxcvbnm"; /* for encoding letter */ char mapd[] = "1357924680"; /* for encoding digit */
...
buf[--n] = 0;
printf("<parent> message [%s] is of length %d\n",buf,n); for (i = 0; i < n; i++) /* encrypt */
             if (buf[i] >= 'a' && buf[i] <= 'z')
               buf[i] = mapl[buf[i]-'a'];
else if (buf[i] >= 'A' && buf[i] <= 'Z') buf[i] = mapl[buf[i]-'A']-('a'-'A');
